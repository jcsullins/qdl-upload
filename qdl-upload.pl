...
my @magicfoo = ('Q','C','O','M',' ','h','i','g','h',' ','s','p','e','e','d',' ','p','r','o','t','o','c','o','l',' ','h','s','t',0,0,0,0);
...
exit 1;
	...
	print "Uploding file...\n";
	uploadFile($fd, 0x2a001000, "upload.bin");
	#uploadFile($fd, 0x2a05f000, "upload.bin");
	#uploadFile($fd, 0x40200000, "upload.bin");
	#uploadFile($fd, 0x80000000, "upload.bin");
	#uploadFile($fd, 0xB0000000, "upload.bin");

if(1)
{
	print "Executing file...\n";
	#execute($fd, 0x00000000);
	execute($fd, 0x2a001000);
	#execute($fd, 0x40200000);
	#execute($fd, 0x80000000);
	#execute($fd, 0x2a05f000);
	#execute($fd, 0x10000000);
	#
	exit 0;
...
sub writeMagic
{
	...
	return undef if(!sendPacket($fd, deserialize("01 51 43 4f 4d 20 68 69 67 68 20 73 70 65 65 64 20 70 72 6f 74 6f 63 6f 6c 20 68 73 74 00 00 00 00 06 06 60 FF FF")));
	return 1;
}
...
sub writeChunk
{
    ...
    my $response;
	#print "Writing chunk...\n";
	if(!sendPacket($fd, deserialize("0f " . serial32($address) . " " . serial16($length) . " " . serialize($chunk)))) {
